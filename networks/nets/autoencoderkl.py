...
class AttentionBlock(nn.Module):
    def __init__(self,
        ...
        self.proj_i = nn.Linear(num_channels, 1)
        self.proj_k = nn.Linear(num_channels, num_channels)
        self.proj_v = nn.Linear(num_channels, num_channels)
    ...
    def forward(self, 
        ...
        c_vector = torch.sum(i*k, dim=1, keepdim=True)

        out = c_vector*v
        out = self.fc(out)
        out = self.dropout(out)

        out = out.transpose(-1, -2).reshape(batch, channel, height, width, depth)
        return out + residual
